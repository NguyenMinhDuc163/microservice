...
**GET /api/employees/{id}/leave-history** - Lấy lịch sử nghỉ phép
- Response: `[ LeaveHistory objects ]`

**POST /api/employees/{id}/leave-history** - Thêm mới lịch sử nghỉ phép
...
- Request body: `{ leaveRequestId, managerId }`
...
2. Leave Request Service gọi `GET /api/employees/{id}` để lấy thông tin nhân viên
...
6. Approval Service gọi `GET /api/employees/{id}/manager` để lấy thông tin quản lý
